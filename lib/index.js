...
const Config = z.object({
  ...
  // Official deepseek-v4-pro pricing (2026-08-13, effective until peakFrom):
  // cache-hit input 0.025, cache-miss input 3, output 6. From `peakFrom`
  // DeepSeek switches to peak/off-peak billing — peak hours are Beijing
  // 9:00-12:00 and 14:00-18:00, off-peak rates are half the peak rates
  // (v4-pro peak: 0.30 / 9 / 27; off-peak: 0.15 / 4.5 / 13.5). Every value
  // is overridable from the profile patch. See the official page:
  // https://api-docs.deepseek.com/zh-cn/quick_start/pricing/
  prices: z.object({
    input: z.number().default(3),
    cacheRead: z.number().default(0.025),
    cacheWrite: z.number().default(3),
    output: z.number().default(6),
    peakFrom: z.string().default("2026-08-17T00:00:00+08:00"),
    offPeak: z.object({
      input: z.number().default(4.5),
      cacheRead: z.number().default(0.15),
      cacheWrite: z.number().default(4.5),
      output: z.number().default(13.5),
    }).default({}),
    peak: z.object({
      input: z.number().default(9),
      cacheRead: z.number().default(0.3),
      cacheWrite: z.number().default(9),
      output: z.number().default(27),
    }).default({}),
  }).default({}),
});
...
function apply(ctx, config) {
  ...
  const priceInput = config.prices?.input ?? 3;
  const priceCacheRead = config.prices?.cacheRead ?? 0.025;
  const priceCacheWrite = config.prices?.cacheWrite ?? 3;
  const priceOutput = config.prices?.output ?? 6;
  // FIX/CHG 2026-08-15 (session-fix): 官方 2026-08-17 00:00(北京) 起改为峰谷定价
  // （高峰 9-12 / 14-18，空闲半价）。会话累计费用按「请求时刻」的单价估算，
  // 并在 /usage 响应里带回 pricing.mode 供前端展示口径。
  const priceFlat = { input: priceInput, cacheRead: priceCacheRead, cacheWrite: priceCacheWrite, output: priceOutput };
  const priceOffPeak = {
    input: config.prices?.offPeak?.input ?? 4.5,
    cacheRead: config.prices?.offPeak?.cacheRead ?? 0.15,
    cacheWrite: config.prices?.offPeak?.cacheWrite ?? 4.5,
    output: config.prices?.offPeak?.output ?? 13.5,
  };
  const pricePeak = {
    input: config.prices?.peak?.input ?? 9,
    cacheRead: config.prices?.peak?.cacheRead ?? 0.3,
    cacheWrite: config.prices?.peak?.cacheWrite ?? 9,
    output: config.prices?.peak?.output ?? 27,
  };
  const peakFrom = config.prices?.peakFrom ?? "2026-08-17T00:00:00+08:00";
  const peakFromMs = (() => {
    const t = new Date(peakFrom).getTime();
    return Number.isNaN(t) ? Infinity : t;
  })();
  function effectivePrices(now = Date.now()) {
    if (now < peakFromMs) return { ...priceFlat, mode: "flat" };
    const bj = new Date(now + 8 * 3600 * 1000); // Beijing = UTC+8
    const hour = bj.getUTCHours();
    const peak = (hour >= 9 && hour < 12) || (hour >= 14 && hour < 18);
    return { ...(peak ? pricePeak : priceOffPeak), mode: peak ? "peak" : "off-peak" };
  }
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (sub === "usage") {
            ...
            let usage = null;
            try {
              usage = sessionProjections.snapshot(session).values?.tokenUsage ?? null;
            } catch { usage = null; }
            ...
            const cost = (tokens.uncachedInput * p.input + tokens.cacheRead * p.cacheRead + tokens.cacheWrite * p.cacheWrite + tokens.output * p.output) / 1e6;
            json(res, 200, { sessionId: sid, tokens, totalTokens, cost, pricing: p });
            return;
          }

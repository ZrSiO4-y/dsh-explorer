	...
	factory: (require) => {
		...
		const store = runtime.defineStore({
			...
			actions: {
				...
				setUsage: (d, usage) => { d.usage = usage; },
		...
		function BalanceChip({ t, useStore }) {
			...
			if (state.phase === "loading") {
				...
				const spend = usage && typeof usage.cost === "number" && usage.cost > 0
					? react.createElement(react.Fragment, null,
						...
						react.createElement("span", { title: usage.totalTokens != null ? t("spendTitle") + " · " + usage.totalTokens + " tokens" : t("spendTitle") }, "💥 ¥" + usage.cost.toFixed(2)))
					: null;
		...
		// ── session usage collector (conversation.composer.dock entry) ─────
		// Invisible: reads the active session id, fetches its token usage/cost,
		// and publishes it into the shared store for the balance chip to show.
		function UsageCollector({ sessionId, actions }) {
			useEffect(() => {
				if (!sessionId) { actions.setUsage(null); return; }
				let dead = false;
				const load = () => {
					fetch("/api/explorer/usage?sessionId=" + encodeURIComponent(sessionId))
						...
						.then((d) => {
							if (dead) return;
							if (d && typeof d.cost === "number") actions.setUsage({ totalTokens: d.totalTokens, cost: d.cost, tokens: d.tokens });
							else actions.setUsage(null);
						})
						.catch(() => { if (!dead) actions.setUsage(null); });
				};
				load();
				const id = window.setInterval(load, 20000);
				return () => { dead = true; window.clearInterval(id); actions.setUsage(null); };
			}, [sessionId, actions]);
			return null;
		}
		...
		function apply(ctx) {
			...
			ctx.effect(() => ctx.slots.register({
				...
				id: "explorer-usage-collector",
				order: 10,
				store,
			}, UsageCollector), "explorer: usage collector");
			ctx.effect(() => ctx.slots.register({
				name: "shell.overlay",

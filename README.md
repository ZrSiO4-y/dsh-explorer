...
VS Code 风格的文件资源管理器插件（DeepSeek Harness / DSH）：侧边栏文件树 + 右侧多标签预览面板（代码/图片/PDF/音视频/Markdown/CSV/Excel/Word/zip），支持文件名搜索、排序过滤、复制粘贴、分屏对比、图片缩放、PPT 转 PDF、余额胶囊、GitHub 版本自动更新提示。官方包零改动，纯插件架构。
...
- **更新检测**：检测到 GitHub 有新版本 tag 时右下角提示升级
...
作者每次发版会在 GitHub 打版本 tag（如 `v0.8.0`），你刷新页面后右下角会自动提示「新版本可用」，重新执行上面的 `dsh plugin add` 覆盖安装即可。
